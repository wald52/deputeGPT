...
export async function createQwen3Runtime(
  modelConfig,
  updateProgress,
  {
    transformersRuntime,
  ...
  if (!transformersRuntime.state.AutoTokenizer || !transformersRuntime.state.Qwen3ForCausalLM) {
    throw new Error('Cette version stable de transformers.js ne fournit pas le support necessaire pour Qwen3.');
  }
  ...
  try {
    ...
    throw new Error(`Impossible de charger le tokenizer Qwen3: ${error.message}`);
  }

  let model;
  try {
    model = await transformersRuntime.state.Qwen3ForCausalLM.from_pretrained(modelConfig.path, {
      device: 'webgpu',
      dtype: modelConfig.dtype,
      progress_callback: info => {
        if (info.status === 'progress') {
          updateProgress(0.3 + ((info.progress || 0) * 0.7), 'Chargement du modele Qwen3');
        }
      }
    });
  } catch (error) {
    const errorDetail = error?.message || error?.toString?.() || JSON.stringify(error) || 'erreur inconnue';
    console.error('Erreur brute chargement Qwen3:', error);
    throw new Error(`Impossible de charger le modele Qwen3 (${modelConfig.selectedQuant?.name || modelConfig.selectedQuant?.id || 'quantization inconnue'}): ${errorDetail}`);
  }
