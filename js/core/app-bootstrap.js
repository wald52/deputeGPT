export function createAppBootstrap({
  ...
  function validateBootstrapDataInternal() {
    ...
    if (!deputesData || deputesData.length === 0) {
      console.error('ERREUR CRITIQUE: aucune donnée de député chargée.');
      ...
      errorBanner.textContent = 'Erreur critique : impossible de charger la liste des députés. Vérifiez votre connexion et rechargez la page.';
      target.prepend(errorBanner);
      return;
    }

    console.debug(`Données chargées : ${deputesData.length} députés.`);
  }
