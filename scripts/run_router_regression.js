...
function createRunner(options, data, harness) {
  ...
  return {
    run() {
      ...
      targetedDeputes.forEach((depute, deputeIndex) => {
        ...
        cases.forEach(testCase => {
          runTestCase(testCase, context, harness, summary, casesStream, coverageState);
        });
      ...
      supplementalCases.forEach(testCase => {
        ...
        runTestCase(testCase.case, context, harness, summary, casesStream, coverageState);
      });
    ...
    steps: [
      {
        ...
        expected: {
          ...
          resultKind: 'clarify',
          messageIncludes: ['Voulez-vous les']
        }
      }
    ]
  };
...
function runTestCase(testCase, context, harness, summary, casesStream, coverageState) {
  const session = createSession(context.depute.id);
  const threadId = `${testCase.id}::${context.depute.id}`;

  testCase.steps.forEach((step, stepIndex) => {
    const actual = executeStep(harness, step, session, context.depute, context.votes);
...
function executeStep(harness, step, session, depute, deputeVotes) {
  ...
  if (route.action === 'deterministic' && !result) {
    ...
    const contextVotes = harness.buildAnalysisContextVotes(route, question, deputeVotes);
